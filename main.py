...
def main():
    ...
    # flow.find_plot_psd([0, 1], [473, 474], plot=True)
    ...
    note = 'nanUpstream'
    # save to Numpy array:
    f_name = f'ignore/tests/particleTracking_n{n_particles}_fullsim_D{D_osrc}_{note}.npy'
    np.save(f_name, test_sim.trajectories)

    # Plot results
    f_path = f'ignore/tests/traj_plot_n{n_particles}_d{round(odor_src.D_osrc, 1)}_{note}'
    ...
    # save to .mat file:
    f_path = f'ignore/tests/ParticleTracking_MSPlumeSim_n{n_particles}_t60s_D{D_osrc}_{note}.mat'
